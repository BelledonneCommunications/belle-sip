...
BELLE_SIP_INSTANCIATE_CUSTOM_VPTR_BEGIN(belle_sip_stream_listening_point_t){
    {BELLE_SIP_VPTR_INIT(belle_sip_stream_listening_point_t, belle_sip_listening_point_t, TRUE),
     (belle_sip_object_destroy_t)belle_sip_stream_listening_point_uninit, NULL, NULL, BELLE_SIP_DEFAULT_BUFSIZE_HINT},
    "TCP",
    stream_create_channel} BELLE_SIP_INSTANCIATE_CUSTOM_VPTR_END
	...
	*family = res->ai_family;
	...
	if (*port == 0) {
		...
		if (err == 0) {
			err = bctbx_getnameinfo((struct sockaddr *)&saddr, saddr_len, NULL, 0, portnum, sizeof(portnum),
...
void belle_sip_stream_listening_point_setup_server_socket(belle_sip_stream_listening_point_t *obj,
                                                          belle_sip_source_func_t on_new_connection_cb) {
	int port = belle_sip_uri_get_port(obj->base.listening_uri);

	obj->server_sock =
	    create_server_socket(belle_sip_uri_get_host(obj->base.listening_uri), &port, &obj->base.ai_family);
	if (obj->server_sock == (belle_sip_socket_t)-1) return;
	belle_sip_uri_set_port(((belle_sip_listening_point_t *)obj)->listening_uri, port);
	if (obj->base.stack->dscp) belle_sip_socket_set_dscp(obj->server_sock, obj->base.ai_family, obj->base.stack->dscp);
...
static int on_new_connection(void *userdata, unsigned int events) {
	...
	if (child == (belle_sip_socket_t)-1) {
		belle_sip_error("Listening point [%p] accept() failed on TCP server socket: %s", lp,
...
void belle_sip_stream_listening_point_init(belle_sip_stream_listening_point_t *obj,
                                           belle_sip_stack_t *s,
...
belle_sip_stream_listening_point_new(belle_sip_stack_t *s, const char *ipaddress, int port) {
