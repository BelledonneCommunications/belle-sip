...
static void channel_process_queue(belle_sip_channel_t *obj);
static char *make_logbuf(belle_sip_channel_t *obj, belle_sip_log_level level, const char *buffer, size_t size, belle_sip_direction_t direction);
	...
	if ((body_len > 0) && (ceh != NULL)) {
		...
		if (BELLE_SIP_OBJECT_IS_INSTANCE_OF(bh, belle_sip_memory_body_handler_t)) {
			...
			if (belle_sip_memory_body_handler_unapply_encoding(mbh, content_encoding) == 0) {
				...
				belle_sip_header_t *xceh = belle_sip_header_create("X-BelleSip-Removed-Content-Encoding", content_encoding);
				...
				*end_of_message=tmp;
				...
				if (obj->input_stream.msg && read_size > 0){
					...
					if (obj->input_stream.msg){
						/* despite read_size is zero, belle_sip_message_parse_raw() might return an empty belle_sip_message_t */
