...
typedef enum belle_sip_refresher_state { started, stopped } belle_sip_refresher_state_t;

typedef enum timer_purpose { NORMAL_REFRESH, RETRY } timer_purpose_t;
...
struct belle_sip_refresher {
	...
	belle_sip_request_t *first_acknowledged_request; /*store first request sucessfully acknowledged, usefull to re-build
	                                                    a dialog if needed*/
	belle_sip_dialog_t
	    *dialog; /*Cannot rely on transaction to store dialog because of belle_sip_transaction_reset_dialog*/
	char *realm;
	int target_expires;
	int session_expires;
...
static void cancel_retry(belle_sip_refresher_t *refresher) {
	if (refresher->timer) {
		belle_sip_main_loop_remove_source(belle_sip_stack_get_main_loop(refresher->transaction->base.provider->stack),
		                                  refresher->timer);
...
static void schedule_timer_at(belle_sip_refresher_t *refresher, int delay, timer_purpose_t purpose) {
	...
	belle_sip_main_loop_add_source(belle_sip_stack_get_main_loop(refresher->transaction->base.provider->stack),
	                               refresher->timer);
}
...
static void process_dialog_terminated(belle_sip_listener_t *user_ctx,
	...
	if (refresher && refresher->transaction &&
	    dialog != belle_sip_transaction_get_dialog(BELLE_SIP_TRANSACTION(refresher->transaction)))
		return; /*not for me*/
...
static int is_contact_address_acurate(const belle_sip_refresher_t *refresher, belle_sip_request_t *request) {
	...
	if ((contact = get_first_contact_in_unknown_state(request))) {
		...
		if (channel_public_port == contact_port && channel_public_ip && contact_ip &&
			...
			belle_sip_message(
			    "Refresher [%p]: contact address [%s:%i] does not match channel address[%s:%i] on channel [%p]",
			    refresher, contact_ip, contact_port, channel_public_ip, channel_public_port,
			    refresher->transaction->base.channel);
			return FALSE;
		}
	} else {
...
static void process_response_event(belle_sip_listener_t *user_ctx, const belle_sip_response_event_t *event) {
	...
	if (response_code >= 200 && response_code < 300) {
		...
		if (strcmp(belle_sip_request_get_method(request), "PUBLISH") == 0) {
			...
			if (etag) {
				belle_sip_header_t *sip_if_match = belle_sip_header_create(
				    "SIP-If-Match", belle_sip_header_extension_get_value(BELLE_SIP_HEADER_EXTENSION(etag)));
		...
		if (refresher->target_expires <= 0) {
			...
			const belle_sip_list_t *contact_list =
			    belle_sip_message_get_headers(BELLE_SIP_MESSAGE(request), BELLE_SIP_CONTACT);
			...
			if (contact_list) {
				for (iterator = head = belle_sip_list_copy(contact_list); iterator != NULL; iterator = iterator->next) {
					...
					if (belle_sip_header_contact_get_expires(contact_for_expire) == 0) {
						belle_sip_message_remove_header_from_ptr(BELLE_SIP_MESSAGE(request),
						                                         BELLE_SIP_HEADER(contact_for_expire));
					}
				}
		...
		if (refresher->state == started) {
			...
			if (is_contact_address_acurate(refresher, request) ||
			    (!belle_sip_provider_nat_helper_enabled(client_transaction->base.provider) ||
			     (contact && belle_sip_parameters_has_parameter(
			                     BELLE_SIP_PARAMETERS(contact),
			                     "pub-gruu")))) { /*Disable nat helper in gruu case. Might not be the best fix, maybe
				                                     better to make reflesh is not mandatory*/
				schedule_timer(refresher);        /*re-arm timer*/
			} else {
				if (!refresher->contact_fixing) {
					belle_sip_message("belle_sip_refresher_start(): refresher [%p] is resubmitting request because "
					                  "contact sent was not correct in original request.",
					                  refresher);
		...
		switch (response_code) {
			...
			case 302:
				...
				if (contact) {
					...
					if (uri && belle_sip_refresher_refresh_internal(refresher, refresher->target_expires, TRUE,
					                                                &refresher->auth_events, uri) == 0)
						return;
				}
				break;
			case 401:
			case 407:
				...
				if (refresher->auth_failures > 1) {
					...
					belle_sip_warning("Authentication is failing constantly, %s",
					                  (refresher->target_expires > 0) ? "will retry later" : "giving up.");
					if (refresher->target_expires > 0) retry_later(refresher);
					refresher->auth_failures = 0; /*reset auth failure*/
					break;
				}
				if (refresher->auth_events) {
					refresher->auth_events = belle_sip_list_free_with_data(
					    refresher->auth_events, (void (*)(void *))belle_sip_auth_event_destroy);
				}
				if (belle_sip_refresher_refresh_internal(refresher, refresher->target_expires, TRUE,
				                                         &refresher->auth_events, NULL) == 0)
			...
			case 412:
				...
				break;
			case 423: {
				belle_sip_header_extension_t *min_expires = BELLE_SIP_HEADER_EXTENSION(
				    belle_sip_message_get_header((belle_sip_message_t *)response, "Min-Expires"));
				if (min_expires) {
	...
	if (refresher->listener)
		refresher->listener(refresher, refresher->user_data, response_code,
		                    belle_sip_response_get_reason_phrase(response), will_retry);
}
...
static void process_timeout(belle_sip_listener_t *user_ctx, const belle_sip_timeout_event_t *event) {
	...
	if (refresher->listener)
		refresher->listener(refresher, refresher->user_data, 408, "timeout", refresher->state == started);
}

static void process_transaction_terminated(belle_sip_listener_t *user_ctx,
...
static void destroy(belle_sip_refresher_t *refresher) {
	...
	belle_sip_provider_remove_internal_sip_listener(refresher->transaction->base.provider,
	                                                BELLE_SIP_LISTENER(refresher));
	belle_sip_object_unref(refresher->transaction);
	refresher->transaction = NULL;
	if (refresher->realm) belle_sip_free(refresher->realm);
	if (refresher->auth_events)
		refresher->auth_events =
		    belle_sip_list_free_with_data(refresher->auth_events, (void (*)(void *))belle_sip_auth_event_destroy);
...
process_dialog_terminated, process_io_error, process_request_event, process_response_event, process_timeout,
    process_transaction_terminated,
    process_auth_requested BELLE_SIP_IMPLEMENT_INTERFACE_END
...
void belle_sip_refresher_set_listener(belle_sip_refresher_t *refresher,
                                      belle_sip_refresher_listener_t listener,
                                      void *user_pointer) {
...
static int belle_sip_refresher_refresh_internal(belle_sip_refresher_t *refresher,
	...
	belle_sip_response_t *old_response =
	    belle_sip_transaction_get_response(BELLE_SIP_TRANSACTION(refresher->transaction));
	...
	if (!dialog) {
		const belle_sip_transaction_state_t state =
		    belle_sip_transaction_get_state(BELLE_SIP_TRANSACTION(refresher->transaction));
		/*create new request*/
		if (belle_sip_transaction_state_is_transient(state)) {
			...
			belle_sip_message("Refresher [%p] already has transaction [%p] in state [%s]", refresher,
			                  refresher->transaction, belle_sip_transaction_state_to_string(state));
			...
			if (strcmp(belle_sip_request_get_method(old_request), "PUBLISH") == 0) {
				...
				request = belle_sip_request_clone_with_body(
				    belle_sip_transaction_get_request(BELLE_SIP_TRANSACTION(refresher->transaction)));
		...
		switch (belle_sip_dialog_get_state(dialog)) {
			case BELLE_SIP_DIALOG_CONFIRMED: {
				...
				if (strcmp(belle_sip_request_get_method(request), "SUBSCRIBE") == 0) {
					...
					if ((content_type =
					    ...
					    strcasecmp("resource-lists+xml", belle_sip_header_content_type_get_subtype(content_type)) ==
					        0) {
						/*rfc5367
						 ...
						 */
						belle_sip_message("Removing body, content type and content length for refresher [%p]",
						                  refresher);
			...
			case BELLE_SIP_DIALOG_TERMINATED: {
				if (refresher->first_acknowledged_request) {
					belle_sip_message("Dialog [%p] is in state terminated, recreating a new one for refresher [%p]",
					                  dialog, refresher);
					request = refresher->first_acknowledged_request;
					belle_sip_header_cseq_set_seq_number(
					    belle_sip_message_get_header_by_type(request, belle_sip_header_cseq_t), 20);
					belle_sip_parameters_remove_parameter(
					    BELLE_SIP_PARAMETERS(belle_sip_message_get_header_by_type(request, belle_sip_header_to_t)),
					    "tag");

					belle_sip_message_set_header((belle_sip_message_t *)request,
					                             (belle_sip_header_t *)belle_sip_provider_create_call_id(prov));
					break;
				}
	...
	if (expires_header) belle_sip_header_expires_set_expires(expires_header, refresher->target_expires);
	...
	/*Dialog may have changed, specially if terminated (I.E timeout). In that case, it will only be autiore-created when
	 * response is received from peer, so in the mean time, we reset it*/
	set_or_update_dialog(refresher, belle_sip_transaction_get_dialog(BELLE_SIP_TRANSACTION(client_transaction)));

	if (belle_sip_client_transaction_send_request_to(
	        client_transaction, requri ? requri : preset_route)) { /*send imediatly to requri in case of redirect*/
		belle_sip_error("Cannot send refresh method [%s] for refresher [%p]", belle_sip_request_get_method(request),
		                refresher);
		return -1;
	}
...
belle_sip_header_contact_t *belle_sip_refresher_get_contact(const belle_sip_refresher_t *refresher) {
	...
	if (contact_header_list) {
		contact_header_list =
		    belle_sip_list_find_custom((belle_sip_list_t *)contact_header_list,
		                               (belle_sip_compare_func)belle_sip_header_contact_not_equals_with_uri_omitting,
		                               (const void *)fixed_local_contact);
		if (!contact_header_list) {
			/*reset header list*/
			contact_header_list = belle_sip_message_get_headers(BELLE_SIP_MESSAGE(response), BELLE_SIP_CONTACT);
			contact_header_list = belle_sip_list_find_custom(
			    (belle_sip_list_t *)contact_header_list,
			    (belle_sip_compare_func)belle_sip_header_contact_not_equals_with_uri_omitting, unfixed_local_contact);
		}
		if (!contact_header_list) {
...
static int set_expires_from_trans(belle_sip_refresher_t *refresher) {
	...
	if (strcmp("REGISTER", belle_sip_request_get_method(request)) == 0 ||
		...
		if (expires_header) refresher->target_expires = belle_sip_header_expires_get_expires(expires_header);
		 ...
		 */
		if (strcmp("REGISTER", belle_sip_request_get_method(request)) == 0) {
			if (!expires_header && (contact_header = belle_sip_message_get_header_by_type(
			                            (belle_sip_message_t *)request, belle_sip_header_contact_t))) {
			...
			if ((contact_header = belle_sip_refresher_get_contact(refresher)) != NULL) {
				...
				refresher->obtained_expires =
				    belle_sip_header_contact_get_expires(BELLE_SIP_HEADER_CONTACT(contact_header));
			}
		}
		if (refresher->obtained_expires == -1) {
			/*no contact with expire or not relevant, looking for Expires header*/
			if (response && (expires_header = (belle_sip_header_expires_t *)belle_sip_message_get_header(
			                     BELLE_SIP_MESSAGE(response), BELLE_SIP_EXPIRES))) {
	...
	if (refresher->state == started) {
		...
		if (refresher->target_expires > 0) {
			...
			if (is_contact_address_acurate(refresher, request)) {
				...
				belle_sip_message("belle_sip_refresher_start(): refresher [%p] is resubmitting request because contact "
				                  "sent was not correct in original request.",
				                  refresher);
				belle_sip_refresher_refresh(refresher, refresher->target_expires);
				return 0;
			}
...
static void belle_sip_refresher_stop_internal(belle_sip_refresher_t *refresher, int cancel_pending_transaction) {
	...
	if (cancel_pending_transaction && refresher->transaction &&
	    belle_sip_transaction_state_is_transient(
	        belle_sip_transaction_get_state(BELLE_SIP_TRANSACTION(refresher->transaction)))) {
		belle_sip_transaction_terminate(
		    BELLE_SIP_TRANSACTION(refresher->transaction)); /*refresher cancelled, no need to continue to retransmit*/
	}
	refresher->state = stopped;
}
...
belle_sip_refresher_t *belle_sip_refresher_new(belle_sip_client_transaction_t *transaction) {
	...
	if (belle_sip_transaction_get_dialog(BELLE_SIP_TRANSACTION(transaction))) {
		...
		/*if dialog is already in state confirmed, store initial request to be able to re-create dialog in case of
		 * expired when trying to refresh again*/
		if (belle_sip_dialog_get_state(belle_sip_transaction_get_dialog(BELLE_SIP_TRANSACTION(transaction))) ==
		    BELLE_SIP_DIALOG_CONFIRMED) {
			if (!refresher->first_acknowledged_request)
				belle_sip_object_ref(refresher->first_acknowledged_request = request);
		}
	}
	belle_sip_provider_add_internal_sip_listener(transaction->base.provider, BELLE_SIP_LISTENER(refresher),
	                                             is_register);
...
static void set_or_update_dialog(belle_sip_refresher_t *refresher, belle_sip_dialog_t *dialog) {
	if (refresher->dialog != dialog) {
		belle_sip_message("refresher [%p] : set_or_update_dialog() current=[%p] new=[%p]", refresher, refresher->dialog,
		                  dialog);
		if (refresher->dialog) {
