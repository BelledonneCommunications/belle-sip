	...
	for(iterator=prov->dialogs;iterator!=NULL;iterator=iterator->next) {
		...
		if (belle_sip_dialog_get_state(dialog) != BELLE_SIP_DIALOG_NULL && _belle_sip_dialog_match(dialog,call_id,local_tag,remote_tag)) {
			if (!returned_dialog)
				...
				/* as local_tag is no more mandatory, finding more then one dialog is less impossible*/
				belle_sip_error("More than 1 dialog is matching, check your app");
				returned_dialog = NULL;
				break;
			}
		}
	}
...
belle_sip_dialog_t* belle_sip_provider_guess_dialog(const belle_sip_provider_t *prov, const char* call_id,const char* remote_tag) {
