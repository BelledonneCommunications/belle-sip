...
static void finalize_transaction(belle_sip_transaction_t *tr) {
	...
	if (state != BELLE_SIP_TRANSACTION_TERMINATED) {
		belle_sip_message("Transaction [%p] still in state [%s], will force termination.", tr,
...
belle_sip_provider_find_matching_pending_subscribe_client_transaction_from_notify_req(belle_sip_provider_t *prov,
                                                                                      belle_sip_request_t *req) {
	belle_sip_list_t *elem;
	if (strcmp("NOTIFY", belle_sip_request_get_method(req)) != 0) {
		belle_sip_error("belle_sip_provider_find_matching_pending_subscribe_client_transaction_from_notify_req "
		                "requires a NOTIFY request, not a [%s], on prov [%p]",
		                belle_sip_request_get_method(req), prov);
	}
	...
	if (t) {
		...
		if (ev.dialog) {
			if (strcmp("ACK", method) == 0) {
				...
				belle_sip_server_transaction_t *tr = belle_sip_provider_create_server_transaction(prov, req);
				belle_sip_server_transaction_send_response(tr, belle_sip_response_create_from_request(req, 491));
				return;
			}
		...
		if (prov->unconditional_answer_enabled &&
		    strcmp("ACK", method) != 0) { /*always answer predefined value (I.E 480 by default)*/
			belle_sip_server_transaction_t *tr = belle_sip_provider_create_server_transaction(prov, req);
			belle_sip_server_transaction_send_response(
			    tr, belle_sip_response_create_from_request(req, prov->unconditional_answer));
			return;
		} else {
...
static void belle_sip_provider_dispatch_response(belle_sip_provider_t *p, belle_sip_response_t *msg) {
	...
	switch (belle_sip_response_get_status_code(msg)) {
		...
		case 407: {
			if (t != NULL) {
				...
				belle_sip_header_authorization_t *authorization = BELLE_SIP_HEADER_AUTHORIZATION(
				    belle_sip_message_get_header_by_type(req, belle_sip_header_proxy_authorization_t));
				if (authorization == NULL)
					authorization = belle_sip_message_get_header_by_type(req, belle_sip_header_authorization_t);
				if (authorization != NULL) {
					nonce = belle_sip_header_authorization_get_nonce(authorization);
					if (nonce != NULL) {
						belle_sip_list_t *auth_context_with_nonce = NULL;
						while ((auth_context_with_nonce = belle_sip_list_find_custom(
						            p->auth_contexts, belle_sip_auth_context_find_by_nonce, nonce)) != NULL) {
	...
	if (t) { /*In some re-connection case, specially over udp, transaction may be found, but without associated
		...
		if (t->base.channel == NULL) {
			...
			chan = belle_sip_provider_get_channel(
			    p, t->next_hop); /*might be faster to get channel directly from upper level*/
			if (chan) {
	 ...
	 */
	if (t && t->base.channel) {
		/*since the add_response may indirectly terminate the transaction, we need to guarantee the transaction is not
		 * freed until full completion*/
		belle_sip_object_ref(t);
...
void belle_sip_provider_dispatch_message(belle_sip_provider_t *prov, belle_sip_message_t *msg) {
	...
	if (belle_sip_message_is_request(msg)) {
		...
		if (message_integrity_verified) {
			...
			belle_sip_message(
			    "response_integrity_checking is disabled, the response is notified despite it is invalid.");
			belle_sip_provider_dispatch_response(prov, (belle_sip_response_t *)msg);
		} else {
 ...
 */
static void
compute_hash_from_invariants(belle_sip_message_t *msg, char *branchid, size_t branchid_size, const char *initial) {
	md5_state_t ctx;
	char tmp[256] = {0};
	uint8_t digest[16];
	...
	if (initial) belle_sip_md5_append(&ctx, (uint8_t *)initial, (int)strlen(initial));
	if (requri) {
		size_t offset = 0;
	...
	if (from_tag) belle_sip_md5_append(&ctx, (uint8_t *)from_tag, (int)strlen(from_tag));
	if (to_tag) belle_sip_md5_append(&ctx, (uint8_t *)to_tag, (int)strlen(to_tag));
...
static char *compute_rfc2543_branch(belle_sip_request_t *req, char *branchid, size_t branchid_size) {
	md5_state_t ctx;
	unsigned int cseq =
	    belle_sip_header_cseq_get_seq_number(belle_sip_message_get_header_by_type(req, belle_sip_header_cseq_t));
	uint8_t digest[16];
	const char *callid =
	    belle_sip_header_call_id_get_call_id(belle_sip_message_get_header_by_type(req, belle_sip_header_call_id_t));
	...
	if (v_branch) belle_sip_md5_append(&ctx, (uint8_t *)v_branch, (int)strlen(v_branch));
...
static void
fix_automatic_header_address(belle_sip_provider_t *prov, belle_sip_channel_t *chan, belle_sip_header_address_t *addr) {
	const char *ip = NULL;
	int port = 0;
	belle_sip_uri_t *uri;
	const char *transport;
	belle_sip_header_contact_t *contact =
	    BELLE_SIP_OBJECT_IS_INSTANCE_OF(addr, belle_sip_header_contact_t) ? (belle_sip_header_contact_t *)addr : NULL;
	...
	if (!belle_sip_uri_is_secure(uri)) belle_sip_uri_set_transport_param(uri, transport);
...
static void channel_on_sending(belle_sip_channel_listener_t *obj, belle_sip_channel_t *chan, belle_sip_message_t *msg) {
	...
	belle_sip_header_content_length_t *content_length =
	    (belle_sip_header_content_length_t *)belle_sip_message_get_header(msg, "Content-Length");
	...
	/*
	 * According to RFC3261, content-length is mandatory for stream based transport, but optional for datagram
	 * transport. However some servers (opensips) are confused when they receive a SIP/UDP packet without Content-Length
	 * (they shouldn't).
	 */
	if (!content_length &&
	    belle_sip_message_get_body_size(msg) ==
	        0 /*if body present, content_length is automatically added at channel level*/
...
static belle_sip_channel_t *_belle_sip_provider_find_channel_using_routable(belle_sip_provider_t *p,
 ...
 * In order to do that, we go through all the channels and ask them their routable uri, and see if it matches the uri
 *passed in argument. This creates a lot of temporary objects and iterates through a potentially long list of routables.
 ...
 * 2- check the listening point's uri first (but need to match the ip address to any local ip if it is INADDR_ANY), then
 *use belle_sip_listening_point_get_channel() to see if a channel is matching. belle_sip_listening_point_get_channel()
 *is not optimized currently but will have to be, so at least we leverage on something that will be optimized.
 **/
...
belle_sip_listening_point_t *belle_sip_provider_get_listening_point(belle_sip_provider_t *p, const char *transport) {
	...
	for (l = p->lps; l != NULL; l = l->next) {
		...
		if (strcasecmp(belle_sip_listening_point_get_transport(lp), transport) == 0) return lp;
	}
	return NULL;
}
...
void belle_sip_provider_add_internal_sip_listener(belle_sip_provider_t *p, belle_sip_listener_t *l, int prepend) {
	if (prepend) p->internal_listeners = belle_sip_list_prepend(p->internal_listeners, l);
	else p->internal_listeners = belle_sip_list_append(p->internal_listeners, l);
}
...
static belle_sip_dialog_t *_belle_sip_provider_find_dialog(const belle_sip_provider_t *prov,
                                                           const char *call_id,
                                                           const char *local_tag,
                                                           const char *remote_tag,
                                                           bool_t local_tag_mandatory) {
	...
	for (iterator = prov->dialogs; iterator != NULL; iterator = iterator->next) {
		...
		if (belle_sip_dialog_get_state(dialog) != BELLE_SIP_DIALOG_NULL &&
		    _belle_sip_dialog_match(dialog, call_id, local_tag, remote_tag)) {
			if (!returned_dialog) returned_dialog = dialog;
			else {
...
belle_sip_dialog_t *belle_sip_provider_find_dialog(const belle_sip_provider_t *prov,
                                                   const char *call_id,
                                                   const char *local_tag,
                                                   const char *remote_tag) {
...
belle_sip_dialog_t *belle_sip_provider_find_dialog_with_remote_tag(const belle_sip_provider_t *prov,
                                                                   const char *call_id,
                                                                   const char *remote_tag) {
...
belle_sip_provider_find_dialog_from_message(belle_sip_provider_t *prov, belle_sip_message_t *msg, int as_uas) {
	...
	if (belle_sip_message_is_request(msg)) {
		...
		if (req->dialog) return req->dialog;
	}
...
void belle_sip_provider_remove_dialog(belle_sip_provider_t *prov, belle_sip_dialog_t *dialog) {
	...
	belle_sip_main_loop_do_later(belle_sip_stack_get_main_loop(prov->stack),
	                             (belle_sip_callback_t)notify_dialog_terminated, ev);
}

belle_sip_client_transaction_t *belle_sip_provider_create_client_transaction(belle_sip_provider_t *prov,
                                                                             belle_sip_request_t *req) {
	const char *method = belle_sip_request_get_method(req);
	belle_sip_client_transaction_t *t;
	belle_sip_client_transaction_t *inv_transaction;
	if (strcmp(method, "INVITE") == 0) t = (belle_sip_client_transaction_t *)belle_sip_ict_new(prov, req);
		...
		if (strcmp(method, "CANCEL") == 0) {
			...
			if (inv_transaction && inv_transaction->next_hop) {
				...
				belle_sip_error("No corresponding ict nor dest found for cancel request attached to transaction [%p]",
				                t);
			}
		}
	}
	belle_sip_transaction_set_dialog((belle_sip_transaction_t *)t, belle_sip_provider_find_dialog_from_message(
	                                                                   prov, (belle_sip_message_t *)req, FALSE));
	belle_sip_request_set_dialog(req, NULL); /*get rid of the reference to the dialog, which is no longer needed in the
	                  message. This is to avoid circular references.*/
	return t;
}

belle_sip_server_transaction_t *belle_sip_provider_create_server_transaction(belle_sip_provider_t *prov,
                                                                             belle_sip_request_t *req) {
	belle_sip_server_transaction_t *t;
	...
	if (strcmp(belle_sip_request_get_method(req), "INVITE") == 0) {
		...
		return NULL;
	} else t = (belle_sip_server_transaction_t *)belle_sip_nist_new(prov, req);
	belle_sip_transaction_set_dialog((belle_sip_transaction_t *)t, belle_sip_provider_find_dialog_from_message(
	                                                                   prov, (belle_sip_message_t *)req, TRUE));
	belle_sip_provider_add_server_transaction(prov, t);
	if (resp) {
		/*the response must be sent after the server transaction is refd by belle_sip_provider_add_server_transaction ,
		 * otherwise through callbacks we'll reach a point where it is unrefed before leaving from this function*/
...
belle_sip_channel_t *belle_sip_provider_get_channel(belle_sip_provider_t *p, const belle_sip_hop_t *hop) {
	...
	if (hop->transport != NULL) {
		...
		if (candidate) {
			...
			if (!chan)
				belle_sip_error("Could not create channel to [%s://%s:%i]", hop->transport, hop->host, hop->port);
			return chan;
		}
	}
...
void belle_sip_provider_send_response(belle_sip_provider_t *p, belle_sip_response_t *resp) {
	...
	belle_sip_channel_t *chan;
	belle_sip_header_to_t *to =
	    (belle_sip_header_to_t *)belle_sip_message_get_header((belle_sip_message_t *)resp, "to");
...
belle_sip_client_transaction_t *belle_sip_provider_find_matching_client_transaction(belle_sip_provider_t *prov,
	...
	belle_sip_header_via_t *via =
	    (belle_sip_header_via_t *)belle_sip_message_get_header((belle_sip_message_t *)resp, "via");
	belle_sip_header_cseq_t *cseq =
	    (belle_sip_header_cseq_t *)belle_sip_message_get_header((belle_sip_message_t *)resp, "cseq");
...
belle_sip_transaction_t *belle_sip_provider_find_matching_transaction(belle_sip_list_t *transactions,
                                                                      belle_sip_request_t *req) {
	struct transaction_matcher matcher;
	belle_sip_header_via_t *via =
	    (belle_sip_header_via_t *)belle_sip_message_get_header((belle_sip_message_t *)req, "via");
...
static void authorization_context_fill_from_auth(authorization_context_t *auth_context,
                                                 belle_sip_header_www_authenticate_t *authenticate,
                                                 belle_sip_uri_t *from_uri) {
	...
	if (result == NULL) {
		...
		for (auth_context_lst = p->auth_contexts; auth_context_lst != NULL; auth_context_lst = auth_context_lst->next) {
			...
			belle_sip_debug("belle_sip_provider_auth: \t[realm=%s] [user_id=%s] [call_id=%s]",
			                ...
			                auth_context->callid ? belle_sip_header_call_id_get_call_id(auth_context->callid)
			                                     : "(null)");
			/* We also verify that user matches in case of multi-account to avoid use nonce from another account. For a
			 * single user, from_uri user id and realm user id COULD be different but we assume here that this is not
			 *the case in order to avoid adding another field in auth_context struct.
			 **/
...
static void belle_sip_provider_update_or_create_auth_context(belle_sip_provider_t *p,
	...
	belle_sip_debug(
	    "belle_sip_provider_auth: no matching auth context, creating one for [realm=%s][user_id=%s][call_id=%s]",
	    belle_sip_header_www_authenticate_get_realm(authenticate)
	        ? belle_sip_header_www_authenticate_get_realm(authenticate)
	        : "(null)",
...
int belle_sip_provider_add_authorization(belle_sip_provider_t *p,
	...
	*/
	...
	if (resp) {
		...
		/*search for proxy authenticate*/
		authenticate_lst = belle_sip_list_concat(
		    authenticate_lst,
		    belle_sip_list_copy(belle_sip_message_get_headers(BELLE_SIP_MESSAGE(resp), BELLE_SIP_PROXY_AUTHENTICATE)));
		/*update auth contexts with authenticate headers from response*/
	...
	belle_sip_debug(
	    "belle_sip_provider_auth: looking an auth context for [method=%s][realm=%s][user_id=%s][call_id=%s]",
	    request_method, realm ? realm : "(null)", from_uri ? belle_sip_uri_get_user(from_uri) : "(null)",
	    call_id ? belle_sip_header_call_id_get_call_id(call_id) : "(null)");
