...
struct _belle_sdp_media_description {
	...
	// RFC5939
	belle_sdp_csup_attribute_t* csup;
	belle_sdp_creq_attribute_t* creq;
	belle_sip_list_t* acaps;
	belle_sip_list_t* tcaps;
	belle_sip_list_t* pcfgs;
	belle_sdp_acfg_attribute_t* acfg;
};
...
belle_sip_error_code belle_sdp_media_description_marshal(belle_sdp_media_description_t* media_description, char* buff, size_t buff_size, size_t *offset) {
	...
	belle_sip_list_t* pcfgs;
	for (pcfgs = media_description->pcfgs; pcfgs != NULL; pcfgs = pcfgs->next){
		error = belle_sdp_pcfg_attribute_marshal((belle_sdp_pcfg_attribute_t*)pcfgs->data, buff, buff_size, offset);
		if (error != BELLE_SIP_OK) return error;
		belle_sip_snprintf(buff, buff_size, offset, "\r\n");
	}

	if (media_description->acfg) {
		error = belle_sdp_acfg_attribute_marshal((belle_sdp_acfg_attribute_t*)(&media_description->acfg),buff,buff_size, offset);
		if (error != BELLE_SIP_OK) return error;
		belle_sip_snprintf(buff, buff_size, offset, "\r\n");
	}
...
void belle_sdp_media_description_add_attribute(belle_sdp_media_description_t* media_description, belle_sdp_attribute_t* attribute) {
...
void belle_sdp_media_description_add_bandwidth(belle_sdp_media_description_t* media_description, belle_sdp_bandwidth_t* bandwidth) {
...
belle_sip_error_code belle_sdp_session_description_marshal(belle_sdp_session_description_t* session_description, char* buff, size_t buff_size, size_t *offset) {
	...
	// RFC5939
	if (session_description->creq) {
		error = belle_sdp_creq_attribute_marshal((belle_sdp_creq_attribute_t*)(&session_description->creq),buff,buff_size, offset);
		if (error != BELLE_SIP_OK) return error;
		belle_sip_snprintf(buff, buff_size, offset, "\r\n");
	}

	if (session_description->csup) {
		error = belle_sdp_csup_attribute_marshal((belle_sdp_csup_attribute_t*)(&session_description->csup),buff,buff_size, offset);
		if (error != BELLE_SIP_OK) return error;
		belle_sip_snprintf(buff, buff_size, offset, "\r\n");
	}

	belle_sip_list_t* acaps;
	for (acaps = session_description->acaps; acaps != NULL; acaps = acaps->next){
		error = belle_sdp_acap_attribute_marshal((belle_sdp_acap_attribute_t*)acaps->data, buff, buff_size, offset);
		if (error != BELLE_SIP_OK) return error;
		belle_sip_snprintf(buff, buff_size, offset, "\r\n");
	}

	belle_sip_list_t* tcaps;
	for (tcaps = session_description->tcaps; tcaps != NULL; tcaps = tcaps->next){
		error = belle_sdp_tcap_attribute_marshal((belle_sdp_tcap_attribute_t*)tcaps->data, buff, buff_size, offset);
		if (error != BELLE_SIP_OK) return error;
		belle_sip_snprintf(buff, buff_size, offset, "\r\n");
	}
...
void belle_sdp_session_description_add_attribute(belle_sdp_session_description_t* session_description, belle_sdp_attribute_t* attribute) {
...
void belle_sdp_session_description_set_csup_attribute(belle_sdp_session_description_t* session_description, belle_sdp_csup_attribute_t* csup) {
	SET_OBJECT(session_description,csup,belle_sdp_csup_attribute_t)
}
void belle_sdp_session_description_set_creq_attribute(belle_sdp_session_description_t* session_description, belle_sdp_creq_attribute_t* creq) {
	SET_OBJECT(session_description,creq,belle_sdp_creq_attribute_t)
}
void belle_sdp_session_description_add_tcap_attribute(belle_sdp_session_description_t* session_description, belle_sdp_tcap_attribute_t* tcap) {
	session_description->tcaps = belle_sip_list_append(session_description->tcaps,belle_sip_object_ref(tcap));
}
void belle_sdp_session_description_add_acap_attribute(belle_sdp_session_description_t* session_description, belle_sdp_acap_attribute_t* acap) {
	session_description->acaps = belle_sip_list_append(session_description->acaps,belle_sip_object_ref(acap));
}
void belle_sdp_media_description_set_acfg_attribute(belle_sdp_media_description_t* media_description, belle_sdp_acfg_attribute_t* acfg) {
	SET_OBJECT(media_description,acfg,belle_sdp_acfg_attribute_t)
}
belle_sdp_acfg_attribute_t* belle_sdp_media_description_get_acfg_attribute(belle_sdp_media_description_t* media_description) {
	return media_description->acfg;
}
void belle_sdp_media_description_add_pcfg_attribute(belle_sdp_media_description_t* media_description, belle_sdp_pcfg_attribute_t* pcfg) {
	media_description->pcfgs = belle_sip_list_append(media_description->pcfgs, belle_sip_object_ref(pcfg));
}
