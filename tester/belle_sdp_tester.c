...
static void test_creq_attribute(void) {
	...
	lAttribute = belle_sdp_creq_attribute_parse("a=creq:cap-v0,foo,bar");
...
static void test_tcap_attribute(void) {
	...
	lAttribute = belle_sdp_tcap_attribute_parse("a=tcap:5 RTP/SAVP RTP/SAVPF");
...
static void test_acap_attribute(void) {
	...
	lAttribute = belle_sdp_acap_attribute_parse("a=acap:3 key-mgmt:mikey AQAFgM");
	BC_ASSERT_STRING_EQUAL(belle_sdp_attribute_get_name(BELLE_SDP_ATTRIBUTE(lAttribute)), "acap");
	//BC_ASSERT_EQUAL(belle_sdp_acap_attribute_get_id(lAttribute), 5, int, "%d");
